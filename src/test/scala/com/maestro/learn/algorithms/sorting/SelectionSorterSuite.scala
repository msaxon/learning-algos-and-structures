...
class SelectionSorterSuite extends AnyWordSpec {

  private val COLLECTION_SIZE = 100000
  private val timer = new UnitTestTimer("SelectionSorter")

  "SelectionSort" when {
    "called with an Array[Int]" should {
      "return a sorted Int array" in {
        val unSortedArr = Array(1,6,4,8,10,3,7,5,2,9)
        assert(CustomAsserts.assertInOrder(SelectionSorter.sort(unSortedArr)))
      }
    }
    "called with a large sorted Array[Int]" should {
      "return a sorted Array[Int]" in {
        ...
        assert(CustomAsserts.assertInOrder(SelectionSorter.sort(preSortedInts)))
    ...
    "called with a large reversed sorted Array[Int]" should {
      "return a sorted Array[Int]" in {
        ...
        assert(CustomAsserts.assertInOrder(SelectionSorter.sort(preSortedReversedInts)))
    ...
    "called with a fully random Array[Int]" should {
      "return a sorted Array[Int]" in {
        ...
        assert(CustomAsserts.assertInOrder(SelectionSorter.sort(randomInts)))
